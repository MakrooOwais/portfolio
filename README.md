...
- `data.yaml`: Contains all the content for the portfolio (experience, research, projects, writing, etc.). Update this file to change the text and links on your website.
- `template.html`: A Jinja2 HTML template. It defines the structure, layout, and styling (using Tailwind CSS) of the website.
- `build.py`: A Python script that reads `data.yaml` and injects its content into `template.html` to generate the final `index.html`.
...
1. **Reads Data:** It uses the `pyyaml` library to open `data.yaml` and load its contents into a Python dictionary.
...
Whenever you update `data.yaml` (to change content) or `template.html` (to change the design), run the following command in your terminal to regenerate your website:
...
- Open `data.yaml` and add a new key-value pair. For example, if you want to add a Twitter link, add `twitter: "https://twitter.com/yourhandle"` under the `hero:` section.
...
- Add a new structured block of data in `data.yaml` (e.g., a list of `certifications`).
